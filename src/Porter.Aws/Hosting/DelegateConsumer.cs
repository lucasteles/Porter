...
sealed class DelegateConsumer<TMessage> : IMessageConsumer<TMessage> where TMessage : notnull
{
    ...
    public static (Type, Type[]) ValidateParams(Delegate handler)
    {
        ...
        if (!funcParams.Contains(typeof(TMessage)))
            throw new PorterException($"No parameter of type {typeof(TMessage).Name} found");
        return (delegateType, funcParams);
    }

    public async Task Consume(TMessage message, IMessageMeta meta, CancellationToken ct)
    {
        ...
        var args = funcParams
            .Select(t =>
                t switch
                {
                    ...
                    _ when t == typeof(CancellationToken) => ct,
