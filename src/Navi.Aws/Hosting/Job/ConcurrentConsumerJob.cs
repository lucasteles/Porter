...
{
    readonly IOptionsMonitor<NaviConfig> config;
    ...
    public ConcurrentConsumerJob(
        ...
        IOptionsMonitor<NaviConfig> config,
        ...
        do
            ...
            catch (Exception ex)
            {
                logger.LogCritical(ex,
                    "Navi: Polling Worker Failure ({DescriberTopicName})",
                    describer.TopicName);
        ...
        CancellationToken stopToken)
    {
        async Task TopicConsumer()
        {
            await foreach (var (message, ctx) in channel.ReadAllAsync(stopToken))
                ...
                {
                    logger.LogCritical(ex,
                        "Navi: Consumer Worker Failure ({DescriberTopicName})",
                        describer.TopicName);
                    if (describer.ErrorHandler is not null)
                        await describer.ErrorHandler(ex);
                    if (config.CurrentValue.RaiseExceptions)
