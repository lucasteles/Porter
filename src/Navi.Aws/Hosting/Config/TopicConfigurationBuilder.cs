    ...
    where TMessage : notnull
{
    ...
    IConsumerDescriber ITopicConfigurationBuilder.CreateConsumerDescriber(
        IServiceProvider sp)
    {
        var settings = sp.GetRequiredService<IOptions<NaviConfig>>().Value;
        var config =
            new ConsumerConfig
            {
        ...
        if (config.PollingInterval <
            ...
            throw new InvalidOperationException(
                $"{nameof(NaviConfig.PollingIntervalInSeconds)} can't be less then {nameof(NaviConfig.LongPollingWaitInSeconds)}");
